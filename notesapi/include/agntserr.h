...
#define ERR_AGENT_NOT_FOUND						(PKG_AGENTS3+42)
	errortext(ERR_AGENT_NOT_FOUND,				"Agent '%s' not found in database '%p'")
#define ERR_AGENT_DESIGNUPDATE_ENABLE			(PKG_AGENTS3+43)
	errortext(ERR_AGENT_DESIGNUPDATE_ENABLE,	"Agent '%s' in '%p' could not be kept enabled during design update from template '%s'. Agent signer '%a'.")
#define ERR_AGENT_DESIGNUPDATE_SERVER			(PKG_AGENTS3+44)
	errortext(ERR_AGENT_DESIGNUPDATE_SERVER,	"Enabled status can be preserved only on the server where the agent is scheduled to run or, for mail agents, on the home mail server. Enabled status cannot be preserved if '-Any server-' is specified for the agent.")
#define ERR_AGENT_DESIGNUPDATE_VERSION			(PKG_AGENTS3+45)
	errortext(ERR_AGENT_DESIGNUPDATE_VERSION,	"Enabled status can be preserved only for agents that were saved with Notes Domino release 6 or later.")
