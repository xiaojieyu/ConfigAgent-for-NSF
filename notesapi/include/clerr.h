...
#ifndef MAC
	errortext(ERR_SERVER_NOT_FOUND,		"Unable to find path to server. Check that your network connection is working. If you have a working connection, go to Preferences - Notes Ports and click Trace to discover where it breaks down.")
#else
	/* SPR EBRL6TDP6J, specific error message on Mac platform, xuhong*/
	errortext(ERR_SERVER_NOT_FOUND,		"Unable to find path to server. To trace this connection, use Lotus Notes - Preferences - User Preferences - Ports - Trace (Notes client) or Trace command (Domino server)")
#endif
...
#define ERR_NO_UDF_CONN_REC				PKG_CLIENT+47
	errortext(ERR_NO_UDF_CONN_REC,	 	"You can not get DB2 information from a pre-R7 server")
