...
/* Before V4, MAXPATH was 256 for MAC, NLM & UNIX.  For others it was 100.
   ...
   MAXPATH define in V4 is now 256 for all 32 bit platforms; on 64bit platforms
   it is 1024 and NDMAXPATH is 4096
*/
...
#define MAXPATH			256			/* Maximum pathname */
	...
	#define NDMAXPATH		4096			/* on 64Bit servers, NDMAXPATH (Internal variable) is now 1024 */
#else
	#define NDMAXPATH		256			/* Internal variable for Maximum pathname */
#endif
...
#define DESIGN_COMPNAME_MAX	256	/* max length of composite app name: datatype, field in datatype, property, namespace, action ID */
...
#define MAILSYSTEM_INTERNET		4
#define MAILSYSTEM_POP			5
#define MAILSYSTEM_IMAP			6
#define MAILSYSTEM_DWA			7
...
/* Settings for themes on design elements, used for several design element types. */
#define THEME_DEFAULT 0	/* same as database setting */
#define THEME_ENABLE 1	/* use themes for this design element even if DB theme option disabled. */
#define THEME_DISABLE 2	/* don't use themes even it db option enabled. */
