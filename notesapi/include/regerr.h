...
#define ERR_REG_NO_SRV_VIEW				PKG_REG + 35
	errortext(ERR_REG_NO_SRV_VIEW, "The Domino Directory you are registering the server to does not contain a $Servers view.  In order to register a server, you must use a Domino Directory.  You can replace the design of this Address Book with the Domino Directory template.")
...
#define SETUP_NETWORK_LAST		(PKG_SETUP+100)
	...
	stringtext(SETUP_NETWORK_FIRST+4, "Windows/64;AIX/64;Linux/64;Solaris/64|3|TCPIP|TCP, 0, 15, 0|TCP/IP")
	...
	stringtext(SETUP_NETWORK_FIRST+7,  "Windows/64;Windows/32;Windows/16;OS/2v2;Netware|15|COM1|XPC, 1, 15, 0|COM1")
	stringtext(SETUP_NETWORK_FIRST+8,  "Windows/64;Windows/32;Windows/16;OS/2v2;Netware|15|COM2|XPC, 2, 15, 0|COM2")
	stringtext(SETUP_NETWORK_FIRST+9,  "Windows/64;Windows/32;Windows/16;OS/2v2;Netware|15|COM3|XPC, 3, 15, 0|COM3")
	stringtext(SETUP_NETWORK_FIRST+10, "Windows/64;Windows/32;Windows/16;OS/2v2;Netware|15|COM4|XPC, 4, 15, 0|COM4")
	stringtext(SETUP_NETWORK_FIRST+11, "Windows/64;Windows/32;Windows/16;OS/2v2;Netware|15|COM5|XPC, 5, 15, 0|COM5")
	stringtext(SETUP_NETWORK_FIRST+12, "UNIX;AIX/64;Linux/64;Solaris/64|15|Serial1|XPC, 1, 15, 0|Serial1")
	stringtext(SETUP_NETWORK_FIRST+13, "UNIX;AIX/64;Linux/64;Solaris/64|15|Serial2|XPC, 2, 15, 0|Serial2")
...
#define	STR_REG_ADDRESSBOOK_TITLE_STRING		(PKG_REG+153)
	stringtext(STR_REG_ADDRESSBOOK_TITLE_STRING,	"%a's Contacts")
...
#define ERR_REG_IMAP_REQUIRES_PASSWORD	(PKG_REG+241)
	errortext(ERR_REG_IMAP_REQUIRES_PASSWORD,	"An internet password is required for users of the chosen mail system.")
...
#define STR_CPPFBWS_TITLE						(PKG_REG2+94)
	stringtext(STR_CPPFBWS_TITLE,				"CPP FreeBusy WebService")
#define	STR_REG_AGENT_RUN_ONBEHALF				(PKG_REG2+95)
	errortext(STR_REG_AGENT_RUN_ONBEHALF,			"%a needs Sign agents to run on behalf of someone else rights on %a in order to have the Administration Process reschedule the agent %s in %s")

#define ERR_REG_MAILSYSTEM_REQUIRES_INETADDR	(PKG_REG2+122)
    errortext(ERR_REG_MAILSYSTEM_REQUIRES_INETADDR,	"An internet address is required for users of the selected mail system.")
#define ERR_REG_MAILSYSTEM_REQUIRES_FWDADDR	(PKG_REG2+123)
    errortext(ERR_REG_MAILSYSTEM_REQUIRES_FWDADDR,		"A forwarding address is required for users of the selected mail system.")
#define ERR_REG_ROAMING_REQUIRES_ID				(PKG_REG2+124)
    errortext(ERR_REG_ROAMING_REQUIRES_ID,		"A Notes ID must be created for a roaming user.")
#define ERR_REG_MAILSYSTEM_NO_ROAMING			(PKG_REG2+125)
    errortext(ERR_REG_MAILSYSTEM_NO_ROAMING,	"A user of the selected mail system may not roam.")
#define ERR_REG_MAILSYSTEM_NO_IDINMAIL			(PKG_REG2+126)
    errortext(ERR_REG_MAILSYSTEM_NO_IDINMAIL,	"A user of the selected mail system may not store a Notes ID in their mail file.")
