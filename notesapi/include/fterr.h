...
#define ERR_FT					PKG_FT+0
	errortext(ERR_FT,			"Unknown full text indexing error")
...
#define ERR_FT_DB_NOT_FOUND		PKG_FT+7
	errortext(ERR_FT_DB_NOT_FOUND,"Application must be full text indexed before search is allowed")
...
#define LOG_FT_LEGACY_ERR		PKG_FT+24
	errortext(LOG_FT_LEGACY_ERR,"The full text index structure for %s is incompatible and is being rebuilt.")
...
// for parsing web queries
#define ERR_FT_NO_WEBQUERY		PKG_FT+27
	errortext(ERR_FT_NO_WEBQUERY, "Query was not converted into web-style syntax.")
...
#define ERR_FT_FIELD_NOT_INDEXED PKG_FT+36
	errortext(ERR_FT_FIELD_NOT_INDEXED,	"The query contains a new number or date field that was added after the full text index was created.  Please recreate the full text index to use this field in queries.")
...
#define ERR_FT_TOPIC_CORRUPT_INDEX	PKG_FT+39
	errortext(ERR_FT_TOPIC_CORRUPT_INDEX,  "The full text index for this database is corrupted.  Delete and recreate the full text index.")
...
#define ERR_FT_KV_INIT					PKG_FT+61
	errortext(ERR_FT_KV_INIT,		"Full Text Error - failure to initialize KV filter")
