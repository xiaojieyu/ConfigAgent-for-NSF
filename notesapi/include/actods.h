...
/*                                                                   */
/* Copyright IBM Corp. 1994, 2006  All Rights Reserved.              */
/*                                                                   */
...
#define		ACTION_SHOW_IN_POPUPMENU		0x10000000L
#define		ACTION_MAKE_SPLIT_BUTTON		0x20000000L /* LI: 4602.02, Provide support for "Split button" for java action bar */
#define		ACTION_ODS_FLAG_MASK			0x35F9F1FF /* need update to include the bitwise flags for the Flags field. ex. one for LI 4602.02 */ 
...
typedef struct 
	...
	WORD	wCompActionIDLen;		/* Length of name of action ID (from composite definition). */
	WORD	wExtra;					/* available */
...
#define ACTION_BAR_FLAG_SUPPRESS_SYS_POPUPS	0x00004000	/* suppress the system actions in the right click pop-up (views only). */
...
typedef struct {
	...
	WORD		wThemeSetting;				/* Hannover Theme Setting */
