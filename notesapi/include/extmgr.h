...
/*	These functions are not actually defined (thus their declarations are only
	...
	STATUS LNPUBLIC ConflictHandler ( 
						DHANDLE hDb,			Database Handle
						DHANDLE hOldNote,	Original Note Handle
						DHANDLE hNewNote,	New Note Handle
	...
	STATUS LNPUBLIC NSFMarkRead(
							...
							DHANDLE		hNote,			- Handle to note being marked read
	...
	that qualify from the source database 
   	...
   	STATUS LNPUBLIC NSFArchiveDeleteNotes( 
			...
			DHANDLE 		hTempIDTable, 	- handle of the id table containing the notes ids to be deleted
	...
	copies them from the source db to the specified destination db  
 	...
 	STATUS LNPUBLIC NSFArchiveCopyNotes(
 		...
 		DHANDLE   hIdTable, 		  - handle to the id table containing the note ids to be copied to the dest
 		DWORD    dwFlags, 		  - none at this time
	...
	STATUS LNPUBLIC NSFDbMediaRecoveryNote(DBHANDLE hDB, DHANDLE hNote, BOOL IsInsert, BOOL IsUpdate, BOOL IsDelete)
