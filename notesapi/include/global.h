...
#endif
#elif defined(MAC) || defined(applec) || defined(__PPCC__) || defined(__MWERKS__) || defined(__SC__) || defined(__MRC__)  || defined(THINK_C) || defined(__APPLE_CC__)
	#ifndef MAC
	#define MAC
	#endif
...
/* The size of this data type varies by platform - on 32bit platforms it is 32bit, on 64bit it is 64bit */
	...
	#if defined(LONGIS64BIT) || defined(UNIX)  /* UNIX 64bit platform long is always 64bit */
		typedef unsigned long NDSIZE_T;	
	#else
		typedef unsigned long long NDSIZE_T;
	#endif
	#define MAXNDSIZE_T (NDSIZE_T) 0xffffffffffffffff
...
#if defined(ND64) || defined(NDUNIX64)  
	#if defined(LONGIS64BIT) || defined(UNIX)  
		typedef unsigned long NDSIZE_T64;	
	#else
		typedef unsigned long long NDSIZE_T64;
	#endif
	#define MAXNDSIZE_T64 (NDSIZE_T64) 0xffffffffffffffff
#else
#if defined(_MSC_VER) && (_MSC_VER < 1300)   /* if 1300 = MSVC7.NET compiler version */
...
#elif defined(NT) && defined(_AMD64_)
/*	Just like va_arg from stdargs.h, but without the increment of _AP.offset	*/
#define VARARG_PEEK(_AP,_TYPE)	\
...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
#if defined(ZLINUX) || defined(LINUX64)
