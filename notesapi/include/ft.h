...
STATUS LNPUBLIC FTOpenSearch(DHANDLE far *rethSearch);
STATUS LNPUBLIC FTSearch(DBHANDLE hDB, DHANDLE far *phSearch, HCOLLECTION hColl,
							char far *Query,
							DWORD Options, WORD Limit, DHANDLE hIDTable,
							DWORD far *retNumDocs,
							DHANDLE far *Reserved,
							DHANDLE far *rethResults);
STATUS LNPUBLIC FTCloseSearch(DHANDLE hSearch);
...
STATUS LNPUBLIC FTSearchExt(	/* Extended full text search */
	...
	DHANDLE far *phSearch, 		/* Previously allocated search handle */
	HCOLLECTION hColl,
	...
	DHANDLE far *Reserved,
	DHANDLE far *rethResults,	/* results format depends on options selected */
	...
	DHANDLE hNames); 			/* User's Name List (NAMES_LIST) or NULL */
