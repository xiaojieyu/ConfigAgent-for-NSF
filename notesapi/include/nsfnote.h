...
STATUS LNPUBLIC NSFItemGetModifiedTime(DHANDLE hNote, const char *ItemName, WORD ItemNameLength, DWORD Flags, TIMEDATE *retTime);
STATUS LNPUBLIC NSFItemGetModifiedTimeByBLOCKID (DHANDLE hNote, BLOCKID bhItem, DWORD Flags, TIMEDATE *retTime);
...
STATUS LNPUBLIC NSFDbNoteLock (DBHANDLE hDB, NOTEID NoteID, DWORD Flags, char *pLockers, DHANDLE *rethLockers, DWORD *retLength);
STATUS LNPUBLIC NSFDbNoteUnlock (DBHANDLE hDB, NOTEID NoteID, DWORD Flags);
STATUS LNPUBLIC NSFNoteOpenWithLock (DBHANDLE hDB, NOTEID NoteID, DWORD LockFlags, DWORD OpenFlags, char *pLockers, DHANDLE *rethLockers, DWORD *retLength, NOTEHANDLE far *rethNote);
...
typedef STATUS (LNCALLBACKPTR NSFFOLDERADDCALLBACK) (void *Param, UNID *NoteUNID, DHANDLE OpBlock, DWORD OpBlockSize);
...
/*	NSF cipher handle - created and used by various NSF functions,
	and destroyed via NSFCipherDestroy. */
typedef MEMHANDLE CIPHERHANDLE;
#define NULLCIPHERHANDLE NULLMEMHANDLE

/*	Destroy a CIPHERHANDLE that was returned as an output argument from another
	NSF function.  Returns NOERROR if phCipher is NULL or *phCipher is
	NULLCIPHERHANDLE. */
STATUS LNPUBLIC NSFCipherDestroy (CIPHERHANDLE *phCipher);
...
/*	The following two functions are preferred to the above three in 8.0.1 and
	later releases, IF the ENCRYPTION_KEY/CIPHERHANDLE argument is non-NULL.
	They support new cryptographic keys and algorithms introduced in 8.0.1
	as well as any from 8.0 and prior releases.
	The reserved arguments are for future use and must be 0 and NULL. */
STATUS LNPUBLIC NSFNoteCipherExtractFile (NOTEHANDLE hNote, BLOCKID bhItem,
						DWORD ExtractFlags, CIPHERHANDLE hDecryptionCipher,
						const char far *FileName,
						DWORD Reserved, void *pReserved);
STATUS LNPUBLIC NSFNoteCipherExtractWithCallback (NOTEHANDLE hNote, BLOCKID bhItem,
						DWORD ExtractFlags, CIPHERHANDLE hDecryptionCipher,
						NOTEEXTRACTCALLBACK pNoteExtractCallback, void far *pParam,
						DWORD Reserved, void *pReserved);
...
BOOL LNPUBLIC NSFNoteHasMIME ( NOTEHANDLE hNote );
BOOL LNPUBLIC NSFNoteHasComposite ( NOTEHANDLE hNote );
BOOL LNPUBLIC NSFNoteHasMIMEPart ( NOTEHANDLE hNote );
BOOL LNPUBLIC NSFNoteHasRFC822Text ( NOTEHANDLE hNote );
STATUS LNPUBLIC NSFNoteSignExt(NOTEHANDLE hNote, 
								const char far *SignatureItemName,
								WORD ItemCount, DHANDLE hItemIDs);
STATUS LNPUBLIC NSFNoteSign (NOTEHANDLE hNote);
STATUS LNPUBLIC NSFComputeObjectDigest(DHANDLE hNote, BLOCKID bItem);
...
STATUS LNPUBLIC NSFHotSpotSign(BYTE *pSource, DWORD dwSourceLength, BYTE *pObject, DWORD dwObjectLength, DHANDLE *hSigData, DWORD *dwSigLength); 
...
/*	The following function is preferred to the above two in 8.0.1 and later
	releases, IF the ENCRYPTION_KEY/CIPHERHANDLE argument is non-NULL.
	It supports new cryptographic keys and algorithms introduced in 8.0.1
	as well as any from 8.0 and prior releases.
	rethCipherForAttachments may be NULL on input if it will not be needed to
	decrypt attachments (e.g. if DECRYPT_ATTACHMENTS_IN_PLACE flag is used).
	If *rethCipherForAttachments is not NULLCIPHERHANDLE on input, it is
	assumed to be a cipher handle from a previous operation, and is destroyed
	using NSFCipherDestroy before written with the new output value.
	The reserved arguments are for future use and must be 0 and NULL. */
STATUS LNPUBLIC NSFNoteCipherDecrypt (NOTEHANDLE hNote, KFHANDLE hKFC, DWORD DecryptFlags, CIPHERHANDLE far *rethCipherForAttachments, DWORD Reserved, void *pReserved);
...
STATUS LNPUBLIC NSFNoteCheck (DHANDLE hNote);
...
STATUS LNPUBLIC NSFNoteLinkFromText(DHANDLE hLinkText, WORD LinkTextLength,
