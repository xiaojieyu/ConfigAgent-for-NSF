...
#define	ERR_COLLECTION_DELETED		PKG_NIF+5
	errortext(ERR_COLLECTION_DELETED, "Informational, rebuild view needed - collection object was deleted (reading %s %s note Title:'%s')")
...
#define ERR_NIF_LDAP_CONVERTING_CERT	PKG_NIF+77
	errortext(ERR_NIF_LDAP_CONVERTING_CERT, "Error converting userCertificate to Notes internal representation for entry %s attribute %s")
...
#define ERR_CMD_SHOW_NIFPOOL		PKG_NIF+89
	stringtext(ERR_CMD_SHOW_NIFPOOL, "NIFPOOL [headers]")
...
#define ERR_DB2_NIF_OPEN_INIT				PKG_NIF2+71
	errortext(ERR_DB2_NIF_OPEN_INIT,			"DB2NIF OpenCollection initialization error")
#define LOG_DDM_COLLECTION_REPLACES			PKG_NIF2+72
	errortext(LOG_DDM_COLLECTION_REPLACES, 	"Database: %s, View: %s, Replaces: %d, NonReplaces: %d, Inserts: %d, Removes:%d, Compares:%d.")
/* Message used in ReloadDirAssist indicating the updates are occurring due to changes in the primary's $Servers Collection 
 * This message replaces "Directory Assistance is reloading internal tables due to change in Primary Address Book"
 */  
#define ERR_DA_UPDATE_SRVLISTINFO			PKG_NIF2+73
	errortext(ERR_DA_UPDATE_SRVLISTINFO,"Directory Assistance is updating server info in internal tables (Server records in primary directory have changed)")

#define ERR_DB2_NIF_BAD_LEVEL0CT			PKG_NIF2+74
	errortext(ERR_DB2_NIF_BAD_LEVEL0CT, 	"DB2 NIF Collection - negative level 0 count")
#define ERR_GBCACHE_CONTAINER_NOT_VALID		PKG_NIF2+75
	errortext(ERR_GBCACHE_CONTAINER_NOT_VALID, 	"gbiCache container failed consistancy check")
#define ERR_NODE_NOT_INITIALIZED			PKG_NIF2+76
	errortext(ERR_NODE_NOT_INITIALIZED, 	"gbiCache container not initialized")
#define ERR_NODE_NOT_LOCKED					PKG_NIF2+77
	errortext(ERR_NODE_NOT_LOCKED,		 	"gbiCache container failed serialization test")
#define ERR_UPDATE_OUT_OF_SEQUENCE			PKG_NIF2+78
	errortext(ERR_UPDATE_OUT_OF_SEQUENCE,	"nliCache processed an out of sequence update")
#define ERR_CONTAINER_TOO_SMALL				PKG_NIF2+79
	errortext(ERR_CONTAINER_TOO_SMALL,		"nliCache not enough memory available in container")
#define ERR_CONECTION_FAILED				PKG_NIF2+80
	errortext(ERR_CONECTION_FAILED,			"nliCache no longer able to update this container")
#define ERR_NLCACHE_FUNCTION_UNKNOWN		PKG_NIF2+81
	errortext(ERR_NLCACHE_FUNCTION_UNKNOWN,	"Unknown function requested from the NAMELookup Cache")
#define NIF_INI_OUT_OF_RANGE				PKG_NIF2+82
	errortext(NIF_INI_OUT_OF_RANGE,			"Requested value %d for NIF ini variable %s out of range - Default: %d, Min: %d, Max: %d")
