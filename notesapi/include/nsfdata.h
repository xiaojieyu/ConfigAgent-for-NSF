...
#define DBHANDLE DHANDLE
#define NOTEHANDLE DHANDLE
#define ITEMDEFTABLEHANDLE DHANDLE
#define ITEMDEFTABLEEXTHANDLE DHANDLE
#define FORMULAHANDLE DHANDLE
...
#define TYPE_SEAL2					31 + CLASS_NOCOMPUTE	/* Canonical form */
...
/* Sealed bulk key Item Data Type rev. 2, for new seals beginning in 8.01 */

/*	ODS format for a TYPE_SEAL2 item is the following:
		WORD - TYPE_SEAL2
		NOTE_SEAL2_HDR record and extended header information
		A sequence of 0 or more: NOTE_SEAL2 records and extended record 
		information
	There may be multiple of these items on a note; each must contain a header.
*/
...
typedef struct {
	...
	DWORD dwFlags;			  /* Flags, undefined bits MBZ -               */
	                          /*   see SEAL2HDR_xxx below                  */
	DWORD dwEntriesThisItem;  /* Number of NOTE_SEAL2s in this $Seal2 item */
	WORD  wReserved;          /* Currently must be zero                    */
	WORD  wBulkKeyLength;	  /* Bulk key length                           */
	WORD  wBulkKeyType;		  /* Bulk key type                             */
	WORD  wAlgType;			  /* Bulk encryption algorithm                 */
	DWORD dwInitVectorLen;	  /* Length of initialization vector, if any   */
	DWORD dwNumExtensions;	  /* Number of header extension records        */
...
/*	Constants for NOTE_SEAL2_HDR.dwVersion */
#define SEAL2HDR_VER_1		    0
#define SEAL2HDR_CUR_VER	    SEAL2HDR_VER_1

/*	Flags for NOTES_SEAL2_HDR.dwFlags: 
*/
/* Bulk key generated and used by a FIPS 140-2 certified crypto library */
#define SEAL2HDR_FIPS_BULK_KEY		0x00000001	


/* Note record descriptor: Used to describe another record, called the 
   target record.  This record can be used to make ODS data extensible.
   The initial use is to allow for current and future extensions to the 
   NOTE_SEAL2_HDR and NOTE_SEAL2 records. 
*/
typedef struct {
...
/*	Bitmask values for NOTE_RECORD_DESC dwFlags field.
	The low 24 bits of flags are available to modify the target record.
	The high 8 bits of flags are universal flags as defined here. 
*/

/*	RECDESC_CRITICAL: Code processing this extension record must not process
	any of the data to which the extension applies if it does not recognize
	the extension record itself.  If this flag is not set, the extension is
	non-critical and may be ignored if it is not recognized.
*/
#define RECDESC_CRITICAL			0x01000000
#define RECDESC_RESERVED_MASK		0xFF000000
...
typedef struct {
	...
	WORD  wAlgType;				/* Encryption algorithm used to encrypt the  */
	                            /*   bulk (data) encryption key (SEC_ai_xxx) */
	TIMEDATE tdSealed;			/* When the seal was created                 */
	DWORD dwEncryptedBulkKeyLength; /* Length of the encrypted bulk data key */
...
/*	Constants for NOTE_SEAL2.dwVersion */
#define SEAL2_VER_1		    0
#define SEAL2_CUR_VER	    SEAL2_VER_1

/*	Flags for NOTES_SEAL2.dwFlags: 
*/
/* Bulk key was encrypted using a FIPS 140-2 certified cryptographic library */
#define SEAL2FLAG_FIPS_KMGMT_KEY	0x00000001	
/* Key management key is a document encryption key (or NEK) */
#define SEAL2FLAG_NAMED_KEY			0x00000002  
/* Key management public key was in an X.509 certificate. */
#define SEAL2FLAG_X509              0x00000004

/*	NOTE_SEAL2 extension records descriptions follow.  They are used for any
	data that is optional or will not appear in every seal, and also may be 
	used for adding new data in the future while retaining backward 
	compatibility. 
*/

/*	NOTE_SEAL2 extension record usage values (NOTE_RECORD_DESC.dwRecordUsage).
	Comment indicates the corresponding C data type of the target record
	(NOTE_RECORD_DESC.dwRecordType). 
*/
#define SEAL2_RU_TARGET_USER_LICENSE	 1	/* LICENSEID: Helps locate the   */
	                                        /*   correct Notes key           */
#define SEAL2_RU_NAMED_KEY				 2	/* char[] (not NULL-terminated)  */ 
	                                        /* needed for SEAL2FLAG_NAMEDKEY */
#define SEAL2_RU_KEY_CREATION_TIME		 3	/* TIMEDATE: Helps locate the    */
	                                        /*   correct Notes key           */
#define SEAL2_RU_KEYHASH		         4	/* BYTE[]: hash used to help     */
	                                        /*   locate the correct key      */
#define SEAL2_RU_INIT_VECTOR			 5	/* BYTE[]: Initialization Vector */

/*	NOTE_SEAL2 extension record flags values (NOTE_RECORD_DESC.dwFlags).  
	These flags all only apply to their specific extensions, but all are
	in the same "namespace" as the universal RECDESC_RESERVED_MASK flags
	defined above. 
*/

/*	For SEAL2_RU_KEY_CREATION_TIME: 
*/
/* Key creation time applies to USA public key (else Intl public key) */
#define SEAL2FLAG_KCT_USA_KEY		0x00000001	

/* For SEAL2_RU_KEYHASH:
*/
/* Hash is SHA1 (RSAPublicKey) to match the X.509 KeyIdentifier       */
#define SEAL2FLAG_KEYHASH_SHA1_PUBLICKEY  0x00000001
/* Hash is SHA-1 of NEK secret key data                               */
#define SEAL2FLAG_KEYHASH_SHA1_NEK        0x00000002
/* Hash is MD5 (SPKI) to match the value stored in incert.c           */
#define SEAL2FLAG_KEYHASH_MD5_SPKI        0x00000004
